export default class Start extends Phaser.Scene {
    ...
    create() {
        ...
        this.chapter1 = this.sound.add('chapter1', { volume: window.GameVolume, loop: true }); // 背景音乐  
        this.chapter1.play();
        this.getcoin = this.sound.add('getcoin', { volume: window.GameVolume, loop: false }); // 获得金币音效
        this.getcoin2 = this.sound.add('getcoin2', { volume: window.GameVolume, loop: false }); // 获得钻石音效
        this.jump_music = this.sound.add('jump_music', { volume: window.GameVolume, loop: false }); // 跳跃音效
        this.spring_music = this.sound.add('spring_music', { volume: window.GameVolume, loop: false }); // 弹跳音效
        this.death_music = this.sound.add('death_music', { volume: window.GameVolume, loop: false }); // 死亡音效
        this.vectory_music = this.sound.add('vectory_music', { volume: 1, loop: false }); // 胜利音效
        ...
        // 创建一个定时器来每秒更新 timeText  
        this.timeEvent = this.time.addEvent({
            delay: 1000, // 每秒执行一次  
            callback: function () {
                // 计算经过的时间（秒）  
                let elapsedTime = (this.time.now / 1000) - this.startTime;
            ...
            callbackScope: this, // 确保 this 指向正确的上下文  
            loop: true // 设置为 true 以使定时器无限循环  
        });
        // 当你想要停止定时器时，调用以下方法  
        this.stopTimer = function () {
            if (this.timeEvent) {
                this.timeEvent.remove();
                this.timeEvent = null; // 可以选择清除引用，以便垃圾回收  
            }
        };

        // create player 
        this.p = this.physics.add.sprite(132, 132, 'hero'); // 使用 physics.add.sprite 而不是 add.sprite  
        this.p.body.setGravityY(1100); // 设置垂直重力  
        this.p.body.setBounceY(0); // 设置 Y 轴上的反弹  
        this.p.body.linearDamping = 1.2; // 直接设置属性  
        this.p.body.setCollideWorldBounds(true); // 设置在场景边界处碰撞  
        ...
        this.map.setCollisionByExclusion([-1], true, this.layer); // -1 是通常用于表示非碰撞瓦片的值 
        ...
        this.map.setCollisionByExclusion([-1], true, this.river); // -1 是通常用于表示非碰撞瓦片的值 
        ...
        enemyLocation.forEach(location => {
            let enemy = this.physics.add.sprite(location.x, location.y, 'enemy', 0);
            enemy.setBounce(1, 0); 
        ...
        if (!this.anims.anims.size) {
            ...
            this.anims.create({
                key: 'enemy_left',
                frames: this.anims.generateFrameNumbers('enemy', { start: 2, end: 0 }),
                frameRate: 6,
                repeat: -1
            });
            this.anims.create({
    ...
    update(time, delta) {
        ...
        this.enemyList.forEach(enemy => {
            ...
            this.physics.add.collider(enemy, this.layer, this.enemyBounce, null, this); // 
            this.physics.add.collider(this.p, enemy, this.beKilled, null, this); // 
        })
